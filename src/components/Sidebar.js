  ...
  return (
    <SidebarWrapper className={activeSidebar}>
      ...
      <NavWrapper>
        ...
        >
          <span className="menu-item__title"> Хочу посмотреть</span>
          <span className="menu-item__counter">{filmsToWatch.length}</span>
        </Link>
        <Link to="/profile" className="menu-item" onClick={activeMenuHandler}>
          <span className="menu-item__title"> Мои фильмы</span>
          <span className="menu-item__counter">{filmsInLibrary.length}</span>
        </Link>
        <span className="watched-title">Просмотренные:</span>
        <Link
          to="/profile/to-review"
          className="menu-item"
          onClick={activeMenuHandler}
        >
          <span className="menu-item__title"> Ожидает отзыва</span>
          <span className="menu-item__counter">{filmsToReview.length}</span>
        </Link>
        <Link
          to="/profile/reviews"
          className="menu-item"
          onClick={activeMenuHandler}
        >
          <span className="menu-item__title"> С отзывом</span>
          <span className="menu-item__counter">{filmsReviewed.length}</span>
        </Link>
      </NavWrapper>
    </SidebarWrapper>
  );
